...
class ATGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.M1 = np.load("./data/geolife/M1.npy")
        self.M2 = np.load("./data/geolife/M2.npy")
        ...
        if embedding_net:
            ...
            self.loc_embedding = nn.Embedding(num_embeddings=self.total_locations, embedding_dim=self.loc_embedding_dim)
        ...
        if function:
            self.M3 = np.load("./data/geolife/M3.npy")
    ...
    def init_hidden(self, batch_size):
        h = torch.LongTensor(torch.zeros((2 if self.bidirectional else 1, batch_size, self.hidden_dim)))
        c = torch.LongTensor(torch.zeros((2 if self.bidirectional else 1, batch_size, self.hidden_dim)))
        if self.device:
    ...
    def sample(self, batch_size, seq_len, x=None):
        ...
        if flag:
            if self.starting_sample == "zero":
                ...
                x = torch.LongTensor(torch.randint(high=self.total_locations, size=(batch_size, 1))).to(self.device)
