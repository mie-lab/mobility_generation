...
def convert_module_to_f16(input):
    """
    Convert primitive modules to float16.
    """
    if isinstance(input, (nn.Conv1d, nn.Conv2d, nn.Conv3d)):
        input.weight.data = input.weight.data.half()
        input.bias.data = input.bias.data.half()


def convert_module_to_f32(input):
    """
    Convert primitive modules to float32, undoing convert_module_to_f16().
    """
    if isinstance(input, (nn.Conv1d, nn.Conv2d, nn.Conv3d)):
        input.weight.data = input.weight.data.float()
        input.bias.data = input.bias.data.float()
